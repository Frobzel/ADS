...
public class BSTree {
    ...
    private void insert(String s,Node node){
		if(s.compareTo(node.getElement()) < 0 && node.getLeft() != null){
			insert(s, node.getLeft());
		}
		
		else if(s.compareTo(node.getElement()) < 0 && node.getLeft() == null){
			Node newNode = new Node(s, null, null, node);
			node.setLeft(newNode);
			size++;
		}
		
		else if(s.compareTo(node.getElement()) > 0 && node.getRight() != null){
    ...
    private Node find(String s,Node node){
		if(node == null)
			...
			else if((s.compareTo(node.getElement()) < 0 && node.getLeft() == null) || (s.compareTo(node.getElement()) > 0 && node.getRight() == null)){
				return null;
			}
